...
import { blogPosts, rooms, services, users } from "../shared/schema";
...
async function seed() {
  ...
  try {
    ...
    await db.insert(users).values({
      ...
      phone: "+1-555-0200",
      role: "customer",
      preferences: JSON.stringify(["wifi", "ac"]),
      is_vip: false,
    });
    ...
    await db.insert(rooms).values([
      {
        ...
        price: "200.00",
        capacity: 3,
        amenities: JSON.stringify(["wifi", "ac", "oceanView", "balcony"]),
        ...
        status: "available",
        description:
          "Phòng suite sang trọng với view biển tuyệt đẹp, ban công riêng và đầy đủ tiện nghi cao cấp.",
      },
      {
        number: "102",
        type: "deluxe",
        price: "150.00",
        capacity: 2,
        amenities: JSON.stringify(["wifi", "ac", "livingRoom"]),
        images: JSON.stringify([
        ...
        status: "available",
        description:
          "Phòng deluxe hiện đại với phòng khách riêng biệt và view thành phố.",
      },
      {
        number: "201",
        type: "standard",
      ...
      },
      {
        ...
        status: "available",
        description:
          "Phòng tổng thống đẳng cấp với 2 phòng ngủ, bếp đầy đủ và view biển panorama.",
      },
    ]);
...
Liên hệ với chúng tôi để được tư vấn và đặt phòng với giá ưu đãi nhất.
          `,
          excerpt:
            "Khám phá khách sạn 5 sao HotelLux với dịch vụ đẳng cấp và tiện nghi cao cấp.",
...
Khám phá nghệ thuật địa phương và triển lãm quốc tế.
          `,
          excerpt:
            "Khám phá 5 địa điểm du lịch hấp dẫn gần khách sạn HotelLux.",
          authorId: adminUser.id,
          published: true,
        },
      ]);
