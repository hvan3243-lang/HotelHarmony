...
- July 04, 2025. Fixed critical Multi-language System bugs: resolved startup errors in i18n.ts syntax, corrected TypeScript interface mismatches in LanguageSelector components, replaced hard-coded Vietnamese text with translation keys in Home component (room names, amenities), added missing translation keys for common terms (wifi, ac, oceanView, etc.), ensuring complete language switching functionality between Vietnamese and English across all UI elements
- July 04, 2025. Created comprehensive PostgreSQL local setup package: removed all MySQL migration files per user request, developed PostgreSQL local setup guide with automatic installation scripts for Windows and Linux, created seed script with sample data (admin/customer accounts, 4 rooms, 5 services, blog posts), comprehensive documentation for running HotelLux locally with PostgreSQL database, maintaining full feature compatibility with Replit environment
- July 05, 2025. Implemented Google OAuth authentication: added passport-google-oauth20 strategy, configured Google OAuth callback handling, created "Login with Google" button with proper translations, automatic user creation from Google profile, JWT token integration for consistency with existing auth system, OAuth callback URL processing to handle successful authentication
- July 05, 2025. Migrated from PostgreSQL to MySQL: converted database schema from PostgreSQL to MySQL syntax, updated all table definitions to use MySQL data types, replaced @neondatabase/serverless with mysql2 package, created MySQL setup guide with local development instructions, configured Visual Studio Code settings for optimal development experience
